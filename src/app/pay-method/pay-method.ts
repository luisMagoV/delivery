...
export class PayMethodPage implements OnInit {
    ...
    @Input() public orderNumber: any;
    radioSelected: any = 'cash';
    promocode: any;
    color = 'transparent';
    ...
    updateTime(type) {
        ...
        let body: any = {
            priority: type,
            origin: this.appConfig.placeToService,
    ...
    approvedOrder() {
        if (this.service.name === 'Compra en tienda') {
            ...
            if (this.radioSelected?.card_token) {
                _body.card = this.radioSelected?.card_token;
            }
                ...
                if (bodyResponse.status === 'ok') {
                    ...
                    setTimeout(() => {
                        ...
                        this.oPushService.sendFCM({
                            title: 'Nueva Orden',
                            body: 'Han realizado un nuevo pedido',
                        }, ['domiciliary_all'], {
                            url: '/available',
                        });
                        // this.router.navigate(['/tabs/status-order'], {
                        this.router.navigate(['/tabs/pedido-steps'], {// Testing this route
                            replaceUrl: true,
                            queryParams: {
                                data: JSON.stringify({
            ...
            if (this.orderNumber !== '') {
                ...
                this.rest.routes('orders/' + this.orderNumber + '/status', 'v1').addDeviceId().withAuth().put({
                    body: {
                        status: 'approved',
                        payType: this.radioSelected?.type || this.radioSelected,
                        card: this.radioSelected?.card_token || this.radioSelected,
                    },
                }).then((orders: any) => {
                    const bodyResponse = orders.json();
    ...
    openPayPage() {
        this.router.navigate(['/tabs/pay-debit']);
    }
